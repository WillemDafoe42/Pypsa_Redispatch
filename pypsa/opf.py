...
def define_passive_branch_constraints(network, snapshots, mode):
    '''
    Defines flow limit constraints for passive branches (lines & transformers)
    Capacity optimization can be turned off by passing additional value: mode = "market"
    :param network:
    :param snapshots:
    :param mode: can be "market", "lopf", or "redispatch": depending on given keyword, capacity optimization is turned off or on)
    :return:
    '''
    # get passive branches of network
    passive_branches = network.passive_branches()s
    # get extendable passive branches from: define_branch_extension_variables
    extendable_branches = passive_branches[passive_branches.s_nom_extendable]
    # get the non-extendable passive branches (fixed constrained life)
    fixed_branches = passive_branches[~ passive_branches.s_nom_extendable]

    # get_switchable_as_dense: Return a Dataframe for a time-varying component attribute with values for all
    # non-time-varying components filled in with the default values for the
    # attribute.
    # return static s_max_pu time series for non-time varying
    ...
    # set maximum upper flow for non extendable branches
    ...
    # set maximum upper flow for extendable branches
    ...
    if mode = "market":
        ...
        # add upper flow constraint to the model
...
def network_lopf_build_model(network, snapshots=None, skip_pre=False,
                             formulation="angles", mode = "market", ptdf_tolerance=0.):
    """
...
def network_lopf(network, snapshots=None, solver_name="glpk", solver_io=None,
                 ...
                 keep_files=False, formulation="angles", mode = "market", ptdf_tolerance=0.,
    ...
    formulation : string
        Formulation of the linear power flow equations to use; must be
        one of ["angles","cycles","kirchhoff","ptdf"]
    mode: Decides which optimization mode is entered ("lopf", "market" or "redispatch"
    ptdf_tolerance : float
        Value below which PTDF entries are ignored
    free_memory : set, default {'pyomo'}
        Any subset of {'pypsa', 'pyomo'}. Allows to stash `pypsa` time-series
    ...
    network_lopf_build_model(network, snapshots, skip_pre=skip_pre,
                             formulation=formulation, mode = mode, ptdf_tolerance=ptdf_tolerance)
